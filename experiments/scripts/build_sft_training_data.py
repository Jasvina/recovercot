...
def state_prompt(state: dict[str, Any]) -> str:
    ...
    parts = [
        ...
        f"State origin: {state.get('state_origin', 'observed')}",
        f"Parent state id: {state.get('parent_state_id')}",
        f"Perturbation type: {state.get('perturbation_type')}",
        f"Perturbation note: {state.get('perturbation_note')}",
